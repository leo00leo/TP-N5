...
void lcdcursor(pantalla_t* setcursor, unsigned int columnas, unsigned int filas){
...
void lcdprint(pantalla_t*led, char *txt){
    while(*txt){
        lcd_enviar(led,*txt);
        txt++;
    }
...
void lcdsend(pantalla_t *lcd, uint8_t dato, int char_0_cmd){
